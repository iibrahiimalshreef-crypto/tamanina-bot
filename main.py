...
# روابط أو معرفات الصور (ضع الـ file_id هنا بعد استخراجه)
...
# دالة لجلب file_id تلقائياً عند إرسال صورة للبوت
async def get_photo_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    photo_id = update.message.photo[-1].file_id
    await update.message.reply_text(
        f"🔑 **تم استخراج file_id الصورة بنجاح!**\n\n`{photo_id}`\n\n"
        "قم بنسخ هذا النص وضعه في الكود مكان صورة الصباح أو المساء.",
        parse_mode='Markdown'
    )

# --- 6. محرك الجدولة المتقدم ---
                ...
                if current_hour == 6 and current_minute < 5:
                    if chat.get("last_morning_sent") != today_str:
                        try:
                            ...
                            await app.bot.send_photo(chat_id=chat_id, photo=MORNING_ATHKAR_IMAGE, caption=caption_text, parse_mode='Markdown')
                            chats_col.update_one({"chat_id": chat_id}, {"$set": {"last_morning_sent": today_str}})
                        except Exception as e:
                            print(f"خطأ أذكار الصباح {chat_id}: {e}")

                # ج) أذكار المساء (5:00 مساءً)
                if current_hour == 17 and current_minute < 5:
                    if chat.get("last_evening_sent") != today_str:
                        try:
                            ...
                            await app.bot.send_photo(chat_id=chat_id, photo=EVENING_ATHKAR_IMAGE, caption=caption_text, parse_mode='Markdown')
